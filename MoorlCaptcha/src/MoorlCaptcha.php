...
class MoorlCaptcha extends Plugin
{
    final public const NAME = 'MoorlCaptcha';
    final public const DATA_CREATED_AT = '2003-10-16 00:00:00.000';
    ...
    public function uninstall(UninstallContext $uninstallContext): void
    {
        ...
        $this->uninstallCaptcha();
    }

    private function updateCaptcha(): void
    {
        ...
        if (empty($configurationValue['moorlCaptchaTo'])) {
            $configurationValue['moorlCaptchaTo'] = [
                'name' => 'moorlCaptchaTo',
                'isActive' => false,
                'config' => [
                    'timeout' => 10,
        ...
        if (empty($configurationValue['moorlCaptchaBw'])) {
            $configurationValue['moorlCaptchaBw'] = [
                'name' => 'moorlCaptchaBw',
                'isActive' => false,
                'config' => [
                    'rules' => [
                        ['name' => 'email,mail,e_mail', 'active' => true, 'description' => 'Block russian email domain', 'regex' => '\.ru$'],
                        ['name' => '*', 'active' => true, 'description' => 'Global url block', 'regex' => '(https:|http:|www\.)\S*'],
                        ['name' => '*', 'active' => true, 'description' => 'Global bad-word block [EN]', 'regex' => '^[a@][s\$][s\$]$
[a@][s\$][s\$]h[o0][l1][e3][s\$]?
b[a@][s\$][t\+][a@]rd 
